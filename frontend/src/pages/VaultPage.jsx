...
import {
  ...
  History, ArrowLeft, Link, Share2, Bell, Clock, Globe, User,
...
// ————— Credential Row (list view) —————
function CredentialRow({ cred, isSelected, onClick, settings }) {
  return (
    <button
    ...
    >
      ...
      {cred.strength != null && (
        <div className="w-10 h-1 rounded-full overflow-hidden flex-shrink-0" style={{ background: 'var(--color-surface-2)' }}>
...
function CredentialDetail({ cred, onEdit, onDelete, onToggleFavorite, isFavorite, settings, onClose }) {
  ...
  const { data: history = [], isLoading: histLoading } = useQuery({
    queryKey: ['cred-history', cred.id],
    queryFn: () => api.get('/audit', { params: { resourceId: cred.id, limit: 30 } }).then(r => r.data.logs || []),
    enabled: showHistory,
  });
  ...
  const Field = ({ icon: Icon, label, value, children }) => (
    <div className="flex items-center py-3" style={{ borderBottom: '1px solid var(--color-border)' }}>
      ...
      <div className="flex-1 min-w-0">{children || (
        <span className="text-sm" style={{ color: value ? 'var(--color-text)' : 'var(--color-muted)' }}>
          {value || '—'}
        </span>
      )}</div>
    </div>
    ...
    <div className="flex flex-col h-full" style={{ background: 'var(--color-bg)' }}>
      ...
      <div className="flex items-center gap-1 px-4 py-2 overflow-x-auto" style={{ borderBottom: '1px solid var(--color-border)', background: 'var(--color-surface)' }}>
        {[
          ...
          { icon: Copy, label: 'Copiar senha', action: () => copyField('password', null), color: 'var(--color-text-muted)' },
          { icon: History, label: 'Histórico', action: () => setShowHistory(!showHistory), color: showHistory ? settings.primaryColor : 'var(--color-text-muted)' },
      ...
      <div className="flex-1 overflow-y-auto p-5">
        {!showHistory ? (
          <>
            ...
            {/* Meta */}
            <div className="mt-4 text-xs space-y-1" style={{ color: 'var(--color-muted)' }}>
              {cred.lastUsed && (
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <span className="text-xs px-2 py-0.5 rounded font-medium"
                          style={{
                            background: log.action.includes('create') ? '#10b98122' : log.action.includes('delete') ? '#ef444422' : '#6366f122',
                            color: log.action.includes('create') ? '#10b981' : log.action.includes('delete') ? '#ef4444' : '#6366f1',
                          }}>
                          {log.action}
                        </span>
                        {log.user && (
...
function CredentialModal({ credential, folders, onClose }) {
  ...
  const mutation = useMutation({
    mutationFn: async () => {
      const encrypted = form.password ? await encryptPassword(form.password, null) : null;
      const payload = {
        title: form.title,
      ...
      if (isEdit) return api.put(`/credentials/${credential.id}`, payload);
      return api.post('/credentials', { ...payload, encryptedPass: payload.encryptedPass || await encryptPassword('', null) });
    },
    onSuccess: () => {
...
export default function VaultPage() {
  ...
  const favMutation = useMutation({
    mutationFn: (credId) => {
      ...
      return isFav
        ? api.delete(`/favorites/${credId}`)
        : api.post(`/favorites/${credId}`);
    },
    onSuccess: () => {
  ...
  // Update selected cred when credentials list refreshes
  useEffect(() => {
    if (selectedCred) {
  ...
  return (
    <div className="flex h-full">
      ...
      {/* Empty state when no item selected and no credentials */}
      {!hasDetail && credentials.length === 0 && !isLoading && !search && (
