...
export default function ProfilePage() {
    ...
    <div>
      ...
      <div className="relative">
        <input type={show ? 'text' : 'password'} value={pwForm[field]}
          ...
          className="w-full bg-[var(--color-surface-2)] border border-[var(--color-border)] rounded-lg px-3 py-2 pr-10 text-[var(--color-text)] text-sm focus:outline-none focus:border-[#C78C00]" />
        <button type="button" onClick={onToggle} className="absolute right-3 top-1/2 -translate-y-1/2 text-[var(--color-muted)] hover:text-[var(--color-text-muted)]">
  ...
  return (
    <div className="p-6 space-y-6 max-w-3xl">
      ...
      {/* Header card */}
      <div className="bg-[var(--color-surface-2)] border border-[var(--color-border)] rounded-xl p-5 flex items-center gap-4">
        ...
        <div className="ml-auto grid grid-cols-2 gap-4 text-center">
          <div>
            <div className="text-xs text-[var(--color-muted)]">Último acesso</div>
            <div className="text-sm text-[var(--color-text-muted)]">{user?.lastLogin ? new Date(user.lastLogin).toLocaleDateString('pt-BR') : '—'}</div>
          </div>
          <div>
            <div className="text-xs text-[var(--color-muted)]">2FA</div>
            <div className={`text-sm font-medium ${user?.totpEnabled ? 'text-green-400' : 'text-[var(--color-muted)]'}`}>
      ...
      <div className="flex gap-1 border-b border-[var(--color-border)] pb-0">
        {TABS.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)}
            className={`px-4 py-2 text-sm font-medium rounded-t-lg transition-colors ${activeTab === tab.id ? 'text-[#C78C00] border-b-2 border-[#C78C00]' : 'text-[var(--color-muted)] hover:text-[var(--color-text-muted)]'}`}>
            {tab.label}
          </button>
        ))}
      </div>
      ...
      {/* Tab: Informações */}
      {activeTab === 'info' && (
        <div className="bg-[var(--color-surface-2)] border border-[var(--color-border)] rounded-xl p-5 space-y-4">
          <h2 className="text-sm font-semibold text-[var(--color-text-muted)] uppercase tracking-wider mb-4">Informações Pessoais</h2>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-sm text-[var(--color-text-muted)] mb-1 block">Nome</label>
            ...
            <div>
              <label className="text-sm text-[var(--color-text-muted)] mb-1 block">Sobrenome</label>
              <input value={profileForm.lastName} onChange={e => setProfileForm({ ...profileForm, lastName: e.target.value })}
                className="w-full bg-[var(--color-surface-2)] border border-[var(--color-border)] rounded-lg px-3 py-2 text-[var(--color-text)] text-sm focus:outline-none focus:border-[#C78C00]" />
            </div>
          </div>
          <div>
            <label className="text-sm text-[var(--color-text-muted)] mb-1 block">E-mail</label>
            <input value={user?.email || ''} disabled
              className="w-full bg-[var(--color-surface-hover)] border border-[var(--color-border)] rounded-lg px-3 py-2 text-[var(--color-muted)] text-sm cursor-not-allowed" />
            <p className="text-xs text-[var(--color-muted)] mt-1">E-mail não pode ser alterado aqui.</p>
          </div>
          <div>
            <label className="text-sm text-[var(--color-text-muted)] mb-1 block">Login</label>
              ...
              {pwForm.newPassword && (
                <div className="space-y-1">
                  <div className="flex gap-1">
                    {[1,2,3,4].map(i => (
                      <div key={i} className={`h-1 flex-1 rounded-full transition-colors ${i <= strength ? strengthColors[strength] : 'bg-[var(--color-surface-hover)]'}`} />
                    ))}
                  </div>
          ...
          <div className="bg-[var(--color-surface-2)] border border-[var(--color-border)] rounded-xl p-5">
            ...
            {user?.totpEnabled ? (
              <div>
                ...
                {twoFaStep === 'disable' ? (
                  <div className="space-y-3">
                    <p className="text-sm text-[var(--color-text-muted)]">Digite o código do seu app autenticador para desativar:</p>
                    <input value={twoFaCode} onChange={e => setTwoFaCode(e.target.value.replace(/\D/g, '').slice(0, 6))} maxLength={6}
                      className="w-32 bg-[var(--color-surface-2)] border border-[var(--color-border)] rounded-lg px-3 py-2 text-[var(--color-text)] text-sm text-center font-mono tracking-widest focus:outline-none focus:border-red-500"
              ...
              <div>
                <p className="text-sm text-[var(--color-text-muted)] mb-4">Proteja sua conta com um app autenticador (Google Authenticator, Authy, etc.)</p>
                  ...
                  <div className="space-y-4">
                    <div className="flex gap-4 items-start">
                      ...
                      <div className="flex-1">
                        <p className="text-sm text-[var(--color-text-muted)] mb-2">1. Escaneie o QR code com seu app</p>
                        <p className="text-sm text-[var(--color-text-muted)] mb-2">2. Ou insira a chave manualmente:</p>
                        <code className="text-xs bg-[var(--color-bg-secondary)] px-2 py-1 rounded font-mono text-[#E7A300] break-all block">{twoFaData.secret}</code>
                        <p className="text-sm text-[var(--color-text-muted)] mt-3 mb-2">3. Digite o código de 6 dígitos gerado:</p>
                        ...
                        <div className="flex gap-2 mt-3">
                          <button onClick={() => { setTwoFaStep(null); setTwoFaData(null); setTwoFaCode(''); }}
                            className="px-3 py-1.5 rounded-lg border border-[var(--color-border)] text-[var(--color-text-muted)] text-sm">Cancelar</button>
      ...
      {activeTab === 'tokens' && (
        <div className="space-y-4">
          ...
          {createdToken && (
            <div className="rounded-xl p-4 border" style={{ background: '#10b98115', borderColor: '#10b98144' }}>
              <div className="flex items-center justify-between mb-2">
                ...
                <button onClick={() => setCreatedToken(null)} className="text-[var(--color-muted)] hover:text-[var(--color-text-muted)]">
                  <X size={14} />
                </button>
              </div>
              <div className="flex items-center gap-2 bg-[var(--color-bg-secondary)] rounded-lg px-3 py-2">
                ...
                <button onClick={() => copyToken(createdToken.token, 'new')}
                  className="flex-shrink-0" style={{ color: copiedId === 'new' ? '#10b981' : 'var(--color-muted)' }}>
                  {copiedId === 'new' ? <Check size={14} /> : <Copy size={14} />}
                </button>
              </div>
              <p className="text-xs text-[var(--color-muted)] mt-1">Este token não será exibido novamente.</p>
            </div>
          )}
          ...
          <div className="bg-[var(--color-surface-2)] border border-[var(--color-border)] rounded-xl p-5">
            ...
            {showCreateToken && (
              <div className="mb-4 p-4 rounded-xl" style={{ background: 'rgba(199,140,0,0.06)', border: '1px solid rgba(199,140,0,0.2)' }}>
                <div className="grid grid-cols-2 gap-3 mb-3">
                  <div>
                    <label className="text-xs text-[var(--color-text-muted)] mb-1 block">Nome do token *</label>
                    <input value={newTokenName} onChange={e => setNewTokenName(e.target.value)}
                      className="w-full bg-[var(--color-surface-2)] border border-[var(--color-border)] rounded-lg px-3 py-2 text-[var(--color-text)] text-sm focus:outline-none focus:border-[#C78C00]"
                      placeholder="Ex: Script de backup" />
                  </div>
                  <div>
                    <label className="text-xs text-[var(--color-text-muted)] mb-1 block">Expira em (opcional)</label>
            ...
            <p className="text-sm text-[var(--color-text-muted)] mb-4">Use tokens para integrar scripts e ferramentas externas com a API.</p>

            {tokensLoading ? (
              <div className="flex items-center gap-2 text-[var(--color-muted)] py-4">
                <RefreshCw size={14} className="animate-spin" /> Carregando...
              </div>
            ) : tokens.length === 0 ? (
              <p className="text-sm text-[var(--color-muted)]">Nenhum token criado ainda.</p>
            ) : (
              <div className="space-y-2">
                {tokens.map(tok => (
                  <div key={tok.id} className="flex items-center justify-between bg-[var(--color-surface-hover)] rounded-xl px-4 py-3">
                    <div>
                      <div className="text-sm font-medium text-[var(--color-text)]">{tok.name}</div>
                      <div className="text-xs text-[var(--color-muted)] font-mono mt-0.5">
                        {tok.tokenPrefix || '—'}…
                        {tok.expiresAt && (
                          <span className={`ml-2 ${new Date(tok.expiresAt) < new Date() ? 'text-red-400' : 'text-[var(--color-muted)]'}`}>
      ...
      {activeTab === 'timeline' && (
        <div className="bg-[var(--color-surface-2)] border border-[var(--color-border)] rounded-xl p-5">
          ...
          {auditLogs.length === 0 ? (
            <p className="text-sm text-[var(--color-muted)]">Nenhuma atividade registrada.</p>
          ) : (
            <div className="space-y-2">
              {auditLogs.map(log => (
                <div key={log.id} className="flex items-start gap-3 py-2 border-b border-[var(--color-border)]">
                  <div className="w-2 h-2 rounded-full bg-[#C78C00] mt-1.5 flex-shrink-0" />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-[var(--color-text-muted)]">{log.action}</span>
                      <span className="text-xs text-[var(--color-muted)]">{new Date(log.createdAt).toLocaleString('pt-BR')}</span>
                    </div>
                    {log.details && (
                      <div className="text-xs text-[var(--color-muted)] mt-0.5 truncate">{JSON.stringify(log.details)}</div>
                    )}
                  </div>
                </div>
              ))}
      ...
      {deleteTokenTarget && (
        <div className="fixed inset-0 flex items-center justify-center z-50 p-4"
          ...
          <div className="w-full max-w-sm rounded-2xl p-6"
            ...
            <h3 className="font-semibold text-[var(--color-text)] mb-2">Revogar token?</h3>
            <p className="text-sm text-[var(--color-text-muted)] mb-5">{deleteTokenTarget.name}</p>
            <div className="flex gap-3">
              <button onClick={() => setDeleteTokenTarget(null)}
                className="flex-1 py-2.5 rounded-xl text-sm hover:bg-[var(--color-surface-2)]"
